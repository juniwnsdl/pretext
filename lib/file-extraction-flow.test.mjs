...
test('stages PDF in Supabase, sends only its path to the upload API, and returns a raw MISO document', async () => {
  const calls = [];
  const staged = [];
  ...
  const document = await extractTextViaMiso(
    file,
    fetchImpl,
    undefined,
    async (candidate) => {
      staged.push(candidate);
      return 'pending/2026-08-14/123e4567-e89b-12d3-a456-426614174000.pdf';
    },
  );
  ...
  assert.deepEqual(JSON.parse(calls[0].options.body), {
    storagePath: 'pending/2026-08-14/123e4567-e89b-12d3-a456-426614174000.pdf',
    fileName: 'policy.pdf',
  });
  ...
  await assert.rejects(
    extractTextViaMiso(
      ...
      async () => {
        call += 1;
        return call === 1
          ? response(true, { fileId: 'f', fileName: 'x.png' })
...
test('an abort between MISO calls prevents the obsolete workflow request', async () => {
  ...
  const calls = [];
  await assert.rejects(
    extractTextViaMiso(new File(['x'], 'x.png'), async (url, options) => {
      calls.push({ url, signal: options.signal });
      return {
        ok: true,
        async json() {
          controller.abort();
          return { success: true, fileId: 'f', fileName: 'x.png' };
        },
      };
    }, controller.signal),
    isAbort,
...
test('forwards abort to the workflow fetch after a completed upload', async () => {
  ...
  const calls = [];
  const extraction = extractTextViaMiso(new File(['x'], 'x.png'), async (url, options) => {
    calls.push({ url, signal: options.signal });
    if (url === '/api/miso/upload') {
      return response(true, { success: true, fileId: 'f', fileName: 'x.png' });
    }
...
test('aborting a pending injected DOCX fallback settles promptly and consumes late outcomes', async () => {
  for (const lateOutcome of ['resolve', 'reject']) {
    const controller = new AbortController();
    const fallback = deferred();
    const fallbackStarted = deferred();
    let misoCalls = 0;
    let deliveredDocument = null;

    const extraction = extractDocxPreferLocal(new File(['docx'], `contract-${lateOutcome}.docx`), {
      local: async () => {
    ...
    assert.equal(outcome, 'AbortError');
    assert.equal(misoCalls, 1);
    assert.equal(deliveredDocument, null);

    if (lateOutcome === 'resolve') {
      fallback.resolve(extracted('miso', 'Late fallback result'));
    } else {
    ...
    assert.equal(await settled, 'AbortError');
    assert.equal(misoCalls, 1);
    assert.equal(deliveredDocument, null);
  }
});
