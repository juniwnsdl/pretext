...
test('successive no-space titled articles split while legal references remain prose', () => {
  const document = textDocument('붙임조문규정.txt', [
    ...
    '제1조에 따른 사항은 본문이다.',
    '제1조의2에 따른 사항도 본문이다.',
    '제1조(목적)에 따른 사항도 본문이다.',
    '제2조(범위)에서 정한 절차도 본문이다.',
    '제3조(보고) 공백 본문을 유지한다.',
    '제4조(삭제)',
  ].join('\n'));
  const result = finalize(document, chunkLawDocument(document));
...
test('parenthesized legal citation suffixes stay prose without rejecting genuine article bodies', () => {
  for (const citation of [
    ...
    assert.equal(parseLegalHeading(citation), null, citation);
  }

  for (const [line, heading, inlineBody] of [
    ['제1조(목적)본문', '제1조(목적)', '본문'],
    ['제1조의2(범위)① 본문', '제1조의2(범위)', '① 본문'],
    ['제2조(보고) 공백 본문', '제2조(보고)', '공백 본문'],
    ['제3조(삭제)', '제3조(삭제)', ''],
  ]) {
    const parsed = parseLegalHeading(line);
    assert.equal(parsed?.kind, 'article', line);
    assert.equal(parsed?.heading, heading, line);
    assert.equal(parsed?.inlineBody, inlineBody, line);
  }
});
...
test('bracketed and decorated official appendix labels become independent legal locations', () => {
  for (const [line, kind] of [
    ['[별표 2] 소속 해양경찰서 위임전결사항', 'appendix'],
    ['[별표2] 소속 해양경찰서 위임전결사항', 'appendix'],
    ['[별지 2] 점검 서식', 'form'],
    ['■ 건축법 시행규칙 [별지 제6호서식] <개정 2025. 1. 1.>', 'form'],
  ]) {
    const parsed = parseLegalHeading(line);
    assert.equal(parsed?.kind, kind, line);
    assert.equal(parsed?.heading, line, line);
  }

  for (const sentence of [
    '[별표 1]에 따른 보호구를 지급하여야 한다.',
    '[별지 제6호서식]을 제출한다.',
    '■ 건축법 시행규칙 [별지 제6호서식]을 제출한다.',
  ]) {
    assert.equal(parseLegalHeading(sentence), null, sentence);
  }

  const document = textDocument('공개법령-별표별지.txt', [
    '제1조(목적)',
    'LAW-BODY-001',
    '[별표 2] 위임전결사항',
    '| 사무 | 담당 |',
    '| --- | --- |',
    '| 보고 | ○ |',
    '■ 건축법 시행규칙 [별지 제6호서식] <개정 2025. 1. 1.>',
    'LAW-FORM-001',
  ].join('\n'));
  const result = finalize(document, chunkLawDocument(document));

  assert.equal(result.chunks.some((chunk) =>
    chunk.includes('[위치] [별표 2] 위임전결사항') && chunk.includes('| 보고 | ○ |')),
  true);
  assert.equal(result.chunks.some((chunk) =>
    chunk.includes('[위치] ■ 건축법 시행규칙 [별지 제6호서식]') && chunk.includes('LAW-FORM-001')),
  true);
  assert.equal(result.chunks.some((chunk) =>
    chunk.includes('[위치] 제1조(목적)') && chunk.includes('LAW-FORM-001')),
  false);
});
...
test('line-initial article citations with spaced connectives stay in the body', () => {
  const document = textDocument('보고규정.txt', [
    '제1조(보고) ① 안전사고 발생 시 즉시 보고하여야 하며, 보고 절차는',
    '제12조 및 제13조에 따라 시행한다.',
    '제5조 제2항의 규정에 따라 월 1회 점검한다.',
    '② 후속 조치는 별도로 정한다.',
  ].join('\n'));
  const result = finalize(document, chunkLawDocument(document));

  assert.equal(result.chunks.length, 1);
  assert.ok(result.chunks[0].includes('[위치] 제1조(보고)'));
  assert.ok(result.chunks[0].includes('제12조 및 제13조에 따라 시행한다.'));
  assert.ok(result.chunks[0].includes('제5조 제2항의 규정에 따라'));
  assert.equal(result.chunks.some((chunk) => chunk.includes('[위치] 제12조')), false);
});

test('spaced 부 칙 is an addendum while sentence-like 부칙/별표 lines stay body', () => {
  const document = textDocument('운영규정.txt', [
    '제1조(목적) 이 규정은 운영 기준을 정한다.',
    '① 약품 취급 작업자에게는',
    '별표 1에 따른 보호구를 지급하여야 한다.',
    '부 칙',
    '제1조(시행일) 이 규정은 공포한 날부터 시행한다.',
  ].join('\n'));
  const result = finalize(document, chunkLawDocument(document));

  assert.ok(result.chunks.some((chunk) => chunk.includes('[위치] 부 칙 > 제1조(시행일)')));
  const firstArticle = result.chunks.find((chunk) => chunk.includes('[위치] 제1조(목적)'));
  assert.ok(firstArticle?.includes('별표 1에 따른 보호구를 지급하여야 한다.'));
  assert.equal(result.chunks.some((chunk) => chunk.includes('[위치] 별표 1에 따른')), false);
});

test('a bare table of contents folds into the real articles without context-only chunks', () => {
  const document = textDocument('정비규정.txt', [
    '목차',
    '제1조(목적)',
    '제2조(적용범위)',
    '부칙',
  ...
  const contextOnly = result.chunks.filter((chunk) => bodyAfterContext(chunk).trim().length === 0);
  assert.equal(contextOnly.length, 0);
  assert.ok(result.chunks.some((chunk) => chunk.includes('이 규정은 정비관리 기준을 정한다.')));
});
