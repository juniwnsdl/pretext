...
test('DOCX manual extraction preserves structured headings and list steps with adjacent safety', async () => {
  ...
  assert.equal(result.processedText.includes('1. 밸브를 연다.'), true);
...
test('blocks unpaired safety before a section, table, and end of file without a safety-only chunk', () => {
  ...
  assert.equal(result.issues.filter((entry) => entry.code === 'manual-safety-without-adjacent-instruction').length, 3);
  assert.equal(result.chunks.some((chunk) => /^\[(?:주의|경고)\][^\n]*$/u.test(bodyAfterContext(chunk).trim())), false);
});
