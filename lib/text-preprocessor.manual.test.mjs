...
test('deep raw-text numbering is preserved but requests review instead of silently flattening', () => {
  ...
  assert.equal(result.resultStatus, 'review');
  assert.equal(result.issues.some((issue) => issue.code === 'manual-deep-numbering-unstructured'), true);
  for (const token of ['2.1 책임과 역할', '2.1.1 주관부서장', '2.1.2 작업자']) {
    assert.equal(result.processedText.split(token).length - 1, 1, token);
  }
});
