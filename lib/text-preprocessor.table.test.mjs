...
test('splits a 4,100-character cell, repeating the row identifier on every fragment chunk', () => {
  const block = {
...
test('keeps a short first cell as the row identifier when a later cell is oversized', () => {
  ...
  const fragmentChunks = result.chunks.filter((chunk) => chunk.includes('행 분할: R-1'));
  assert.equal(result.processedText.match(/R-1/g)?.length, fragmentChunks.length);
});
