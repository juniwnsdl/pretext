...
test('explains hierarchical table headers, formulas, and equivalent converted document tables', () => {
  const guidance = TABLE_HANDLING_GUIDANCE.map((item) => `${item.title} ${item.description}`).join('\n');

  assert.match(guidance, /상위·하위 제목.*>/u);
  ...
  assert.match(guidance, /HWP.*직접 지원하지/u);
});
