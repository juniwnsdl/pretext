...
import * as generalChunker from './preprocessing/general-chunker.ts';
...
for (const romanParent of ['I. \uc0ac\uc5c5 \uac1c\uc694', '\u2161 \uc0ac\uc5c5 \uac1c\uc694']) {
  test(`Roman parent keeps its two-level context for ${romanParent}`, () => {
    const document = {
      version: 1,
      fileName: 'roman-parent.txt',
      sourceFormat: 'txt',
      extractionMethod: 'local-text',
      warnings: [],
      blocks: [
        { id: 'roman-parent', kind: 'raw-text', order: 0, headingPath: [], text: romanParent },
        { id: 'numbered-one', kind: 'raw-text', order: 1, headingPath: [], text: '1. \ucd94\uc9c4 \ubc30\uacbd' },
        { id: 'first-body', kind: 'raw-text', order: 2, headingPath: [], text: '\ucd94\uc9c4 \ubc30\uacbd \ubcf8\ubb38' },
        { id: 'numbered-dotted', kind: 'raw-text', order: 3, headingPath: [], text: '1.1 \ucd94\uc9c4 \uacb0\uacfc' },
        { id: 'second-body', kind: 'raw-text', order: 4, headingPath: [], text: '\ucd94\uc9c4 \uacb0\uacfc \ubcf8\ubb38' },
      ],
    };
    ...
    assert.ok(result.chunks[0].includes(`[\uc139\uc158] ${romanParent} > 1. \ucd94\uc9c4 \ubc30\uacbd`));
    assert.ok(result.chunks[1].includes(`[\uc139\uc158] ${romanParent} > 1.1 \ucd94\uc9c4 \uacb0\uacfc`));
    assert.equal(bodyAfterContext(result.chunks[0]).includes(romanParent), false);
    assert.equal(bodyAfterContext(result.chunks[1]).includes(romanParent), false);
    assert.equal(result.issues.some((issue) => issue.code === 'source-block-consumption-mismatch'), false);

    const output = generalChunker.chunkGeneralDocument(document);
    const firstChild = output.drafts.find((draft) => draft.body.includes('\ucd94\uc9c4 \ubc30\uacbd \ubcf8\ubb38'));
    const secondChild = output.drafts.find((draft) => draft.body.includes('\ucd94\uc9c4 \uacb0\uacfc \ubcf8\ubb38'));
    assert.deepEqual(firstChild?.sourceBlockIds, ['roman-parent', 'numbered-one', 'first-body']);
    assert.deepEqual(secondChild?.sourceBlockIds, ['numbered-dotted', 'second-body']);
  });
}

test('Roman parent with body claims its source before the first child', () => {
  const output = generalChunker.chunkGeneralDocument({
    version: 1,
    fileName: 'roman-parent-body.txt',
    sourceFormat: 'txt',
    extractionMethod: 'local-text',
    warnings: [],
    blocks: [
      { id: 'roman', kind: 'raw-text', order: 0, headingPath: [], text: 'I. \uc0ac\uc5c5 \uac1c\uc694' },
      { id: 'parent-body', kind: 'raw-text', order: 1, headingPath: [], text: '\uc0c1\uc704 \ubcf8\ubb38' },
      { id: 'child', kind: 'raw-text', order: 2, headingPath: [], text: '1. \ucd94\uc9c4 \ubc30\uacbd' },
      { id: 'child-body', kind: 'raw-text', order: 3, headingPath: [], text: '\ud558\uc704 \ubcf8\ubb38' },
    ],
  });
  const parent = output.drafts.find((draft) => draft.body.includes('\uc0c1\uc704 \ubcf8\ubb38'));
  const child = output.drafts.find((draft) => draft.body.includes('\ud558\uc704 \ubcf8\ubb38'));

  assert.deepEqual(parent?.sourceBlockIds, ['roman', 'parent-body']);
  assert.deepEqual(child?.sourceBlockIds, ['child', 'child-body']);
});

test('replaced body-less Roman parents do not fold into the final child body', () => {
  const document = {
    version: 1,
    fileName: 'roman-replacement.txt',
    sourceFormat: 'txt',
    extractionMethod: 'local-text',
    warnings: [],
    blocks: [
      { id: 'roman-one', kind: 'raw-text', order: 0, headingPath: [], text: 'I. \ucd08\uae30 \uac1c\uc694' },
      { id: 'roman-two', kind: 'raw-text', order: 1, headingPath: [], text: 'II. \ub300\uccb4 \uac1c\uc694' },
      { id: 'roman-three', kind: 'raw-text', order: 2, headingPath: [], text: 'III. \ucd5c\uc885 \uac1c\uc694' },
      { id: 'child', kind: 'raw-text', order: 3, headingPath: [], text: '1. \uc0c1\uc138 \ub0b4\uc6a9' },
      { id: 'body', kind: 'raw-text', order: 4, headingPath: [], text: '\uc0c1\uc138 \ubcf8\ubb38' },
    ],
  };
  const result = textPreprocessor.preprocessExtractedDocument(document, 'general');
  const output = generalChunker.chunkGeneralDocument(document);
  ...
  assert.ok(result.chunks[0].includes('[\uc139\uc158] III. \ucd5c\uc885 \uac1c\uc694 > 1. \uc0c1\uc138 \ub0b4\uc6a9'));
  assert.equal(bodyAfterContext(result.chunks[0]), '\uc0c1\uc138 \ubcf8\ubb38');
  assert.deepEqual(output.drafts[0]?.sourceBlockIds, ['roman-one', 'roman-two', 'roman-three', 'child', 'body']);
  assert.equal(result.issues.some((issue) => issue.code === 'source-block-consumption-mismatch'), false);
});

test('structured heading blocks reset Roman inference even when their text is Roman-shaped', () => {
  const result = textPreprocessor.preprocessExtractedDocument({
    version: 1,
    fileName: 'structured-heading.txt',
    sourceFormat: 'docx',
    extractionMethod: 'local-docx',
    warnings: [],
    blocks: [
      { id: 'heading', kind: 'heading', order: 0, headingPath: [], level: 1, text: 'I. \uc0ac\uc5c5 \uac1c\uc694' },
      { id: 'numbered', kind: 'raw-text', order: 1, headingPath: [], text: '1. \ucd94\uc9c4 \ubc30\uacbd' },
      { id: 'body', kind: 'raw-text', order: 2, headingPath: [], text: '\ucd94\uc9c4 \ubc30\uacbd \ubcf8\ubb38' },
    ],
  }, 'general');
  const child = result.chunks.find((chunk) => chunk.includes('\ucd94\uc9c4 \ubc30\uacbd \ubcf8\ubb38'));

  assert.ok(child?.includes('[\uc139\uc158] 1. \ucd94\uc9c4 \ubc30\uacbd'));
  assert.equal(child?.includes('I. \uc0ac\uc5c5 \uac1c\uc694 > 1. \ucd94\uc9c4 \ubc30\uacbd'), false);
  assert.equal(result.issues.some((issue) => issue.code === 'source-block-consumption-mismatch'), false);
});

test('explicit paths consume a body-less Roman parent without a context-only draft', () => {
  const paragraphDocument = {
    version: 1,
    fileName: 'explicit-paragraph.txt',
    sourceFormat: 'docx',
    extractionMethod: 'local-docx',
    warnings: [],
    blocks: [
      { id: 'roman', kind: 'raw-text', order: 0, headingPath: [], text: 'I. \uc0ac\uc5c5 \uac1c\uc694' },
      { id: 'paragraph', kind: 'paragraph', order: 1, headingPath: ['\uba85\uc2dc \uacbd\ub85c'], text: '\uba85\uc2dc \ubcf8\ubb38' },
    ],
  };
  const tableDocument = {
    ...paragraphDocument,
    fileName: 'explicit-table.txt',
    blocks: [
      { id: 'roman', kind: 'raw-text', order: 0, headingPath: [], text: 'I. \uc0ac\uc5c5 \uac1c\uc694' },
      { id: 'table', kind: 'table', order: 1, headingPath: ['\uba85\uc2dc \ud45c \uacbd\ub85c'], rows: [['\ud56d\ubaa9', '\uac12'], ['\uac00', '1']] },
    ],
  };

  for (const document of [paragraphDocument, tableDocument]) {
    const result = textPreprocessor.preprocessExtractedDocument(document, 'general');
    const output = generalChunker.chunkGeneralDocument(document);

    assert.equal(result.chunks.length, 1, document.fileName);
    assert.equal(bodyAfterContext(result.chunks[0]).includes('I. \uc0ac\uc5c5 \uac1c\uc694'), false, document.fileName);
    assert.equal(output.drafts.some((draft) => draft.body.trim().length === 0), false, document.fileName);
    assert.deepEqual(output.drafts[0]?.sourceBlockIds, ['roman', document.blocks[1].id], document.fileName);
    assert.equal(result.issues.some((issue) => issue.code === 'source-block-consumption-mismatch'), false, document.fileName);
  }
});

for (const kind of ['paragraph', 'raw-text']) {
  test(`explicit paths win over heading-like lines inside ${kind} blocks`, () => {
    const result = textPreprocessor.preprocessExtractedDocument({
      version: 1,
      fileName: `explicit-${kind}.docx`,
      sourceFormat: 'docx',
      extractionMethod: 'local-docx',
      warnings: [],
      blocks: [{
        ...
        kind,
        order: 0,
        headingPath: ['명시 상위', '명시 하위'],
        text: 'I. 자동 상위\n1. 자동 하위\n실제 본문',
      }],
    }, 'general');

    assert.equal(result.chunks.length, 1);
    assert.ok(result.chunks[0].includes('[섹션] 명시 상위 > 명시 하위'));
    assert.equal(
      bodyAfterContext(result.chunks[0]),
      'I. 자동 상위\n1. 자동 하위\n실제 본문',
    );
    assert.equal(result.issues.some((issue) => issue.code === 'source-block-consumption-mismatch'), false);
  });
...
test('same explicit path clears Roman inference before its body is consumed', () => {
  const result = textPreprocessor.preprocessExtractedDocument({
    version: 1,
    fileName: 'same-path.txt',
    sourceFormat: 'docx',
    extractionMethod: 'local-docx',
    warnings: [],
    blocks: [
      { id: 'roman', kind: 'raw-text', order: 0, headingPath: [], text: 'I. \uc0ac\uc5c5 \uac1c\uc694' },
      { id: 'explicit', kind: 'paragraph', order: 1, headingPath: ['I. \uc0ac\uc5c5 \uac1c\uc694'], text: '\uba85\uc2dc \ubcf8\ubb38' },
      { id: 'numbered', kind: 'raw-text', order: 2, headingPath: [], text: '1. \ucd94\uc9c4 \ubc30\uacbd' },
      { id: 'body', kind: 'raw-text', order: 3, headingPath: [], text: '\ud558\uc704 \ubcf8\ubb38' },
    ],
  }, 'general');
  const child = result.chunks.find((chunk) => chunk.includes('\ud558\uc704 \ubcf8\ubb38'));
...
test('Roman parent replacement, resets, and tables never retain a stale parent', () => {
  const document = {
    version: 1,
    fileName: 'roman-boundaries.txt',
    sourceFormat: 'txt',
    extractionMethod: 'local-text',
    warnings: [],
    blocks: [
      { id: 'roman-one', kind: 'raw-text', order: 0, headingPath: [], text: 'I. \uc0ac\uc5c5 \uac1c\uc694' },
      { id: 'one', kind: 'raw-text', order: 1, headingPath: [], text: '1. \ucd94\uc9c4 \ubc30\uacbd' },
      { id: 'one-body', kind: 'raw-text', order: 2, headingPath: [], text: '\ucd94\uc9c4 \ubc30\uacbd \ubcf8\ubb38' },
      { id: 'roman-two', kind: 'raw-text', order: 3, headingPath: [], text: '\u2161 \ub300\uccb4 \uac1c\uc694' },
      { id: 'two', kind: 'raw-text', order: 4, headingPath: [], text: '2. \ub300\uccb4 \ub0b4\uc6a9' },
      { id: 'two-body', kind: 'raw-text', order: 5, headingPath: [], text: '\ub300\uccb4 \ub0b4\uc6a9 \ubcf8\ubb38' },
      { id: 'table', kind: 'table', order: 6, headingPath: [], rows: [['\ud56d\ubaa9', '\uac12'], ['\uac00', '1']] },
      { id: 'markdown', kind: 'raw-text', order: 7, headingPath: [], text: '# Markdown \uacbd\uacc4' },
      { id: 'three', kind: 'raw-text', order: 8, headingPath: [], text: '3. Markdown \uc774\ud6c4' },
      { id: 'three-body', kind: 'raw-text', order: 9, headingPath: [], text: 'Markdown \uc774\ud6c4 \ubcf8\ubb38' },
      { id: 'roman-three', kind: 'raw-text', order: 10, headingPath: [], text: 'III. \ub2e4\uc2dc \uc0c1\uc704' },
      { id: 'four', kind: 'raw-text', order: 11, headingPath: [], text: '4. \uad6c\uc870 \uc804' },
      { id: 'structured', kind: 'paragraph', order: 12, headingPath: ['\uba85\uc2dc \uacbd\ub85c'], text: '\uad6c\uc870\ud654 \ubcf8\ubb38' },
      { id: 'six', kind: 'raw-text', order: 13, headingPath: [], text: '6. \uad6c\uc870 \uc774\ud6c4' },
      { id: 'six-body', kind: 'raw-text', order: 14, headingPath: [], text: '\uad6c\uc870 \uc774\ud6c4 \ubcf8\ubb38' },
    ],
  };

  const result = textPreprocessor.preprocessExtractedDocument(document, 'general');
  const chunkFor = (text) => result.chunks.find((chunk) => chunk.includes(text));

  assert.ok(chunkFor('\ub300\uccb4 \ub0b4\uc6a9 \ubcf8\ubb38')?.includes('[\uc139\uc158] \u2161 \ub300\uccb4 \uac1c\uc694 > 2. \ub300\uccb4 \ub0b4\uc6a9'));
  assert.ok(chunkFor('| \uac00 | 1 |')?.includes('[\uc139\uc158] \u2161 \ub300\uccb4 \uac1c\uc694 > 2. \ub300\uccb4 \ub0b4\uc6a9'));
  for (const text of ['Markdown \uc774\ud6c4 \ubcf8\ubb38', '\uad6c\uc870 \uc774\ud6c4 \ubcf8\ubb38']) {
    assert.equal(chunkFor(text)?.includes('> 3.') || chunkFor(text)?.includes('> 5.') || chunkFor(text)?.includes('> 6.'), false, text);
  }
  assert.ok(chunkFor('Markdown \uc774\ud6c4 \ubcf8\ubb38')?.includes('[\uc139\uc158] 3. Markdown \uc774\ud6c4'));
  assert.ok(chunkFor('\uad6c\uc870 \uc774\ud6c4 \ubcf8\ubb38')?.includes('[\uc139\uc158] 6. \uad6c\uc870 \uc774\ud6c4'));
  assert.equal(result.issues.some((issue) => issue.code === 'source-block-consumption-mismatch'), false);
});

test('Roman parent resets at legal, chapter, English structural, and form headings', () => {
  const boundaries = ['\uc81c1\uc870 (\ubaa9\uc801)', '\uc81c1\uc7a5 \ucd1d\uce59', 'Chapter 2.', '[\uc11c\uc2dd \uc81c1\ud638]'];

  for (const [index, boundary] of boundaries.entries()) {
    const output = generalChunker.chunkGeneralDocument({
      version: 1,
      fileName: `boundary-${index}.txt`,
      sourceFormat: 'txt',
      extractionMethod: 'local-text',
      warnings: [],
      blocks: [
        ...
        { id: 'numbered', kind: 'raw-text', order: 1, headingPath: [], text: '1. \ucd94\uc9c4 \ubc30\uacbd' },
        { id: 'body', kind: 'raw-text', order: 2, headingPath: [], text: '\ucd94\uc9c4 \ubc30\uacbd \ubcf8\ubb38' },
        { id: 'boundary', kind: 'raw-text', order: 3, headingPath: [], text: boundary },
        { id: 'after', kind: 'raw-text', order: 4, headingPath: [], text: `\uacbd\uacc4 \uc774\ud6c4 \ubcf8\ubb38-${index}` },
      ],
    });
    const after = output.drafts.find((draft) => draft.body.includes(`\uacbd\uacc4 \uc774\ud6c4 \ubcf8\ubb38-${index}`));

    assert.ok(after, boundary);
    assert.equal(after.contextLines[1].includes('I. \uc0ac\uc5c5 \uac1c\uc694'), false, boundary);
  }
});

test('Roman parent retains false-positive lines as the numbered child body', () => {
  const input = [
    'I. \uc0ac\uc5c5 \uac1c\uc694',
    '1. \ucd94\uc9c4 \ubc30\uacbd',
    'C 70% \ud3c9\uac00 \uae30\uc900',
  ...
  assert.equal(result.chunks.length, 1);
  assert.ok(result.chunks[0].includes('[\uc139\uc158] I. \uc0ac\uc5c5 \uac1c\uc694 > 1. \ucd94\uc9c4 \ubc30\uacbd'));
  for (const text of ['C 70%', 'D 55%', 'PPT', 'A4', '\uac00.', '\ub098.', '1. \ubaa9\ub85d', '3. \ubaa9\ub85d']) {
    assert.ok(result.chunks[0].includes(text), text);
  }
  assert.equal(result.chunks[0].includes('[\uc139\uc158] C 70%'), false);
  assert.equal(result.chunks[0].includes('[\uc139\uc158] 2. \ubaa9\ub85d'), false);
});
...
test('official RFP Roman headings and numbered form labels split without promoting rating codes', () => {
  const input = [
    'I 제안 내용',
    '제안 본문입니다.',
    'C 70% 평가항목별 내용이 적정함',
    'D 55% 평가항목별 내용에 보완이 필요함',
    'PPT파일 형식, A4 규격',
    '[서식 제1호]',
    '서식 본문입니다.',
    '[서식 제6호-1]',
  ...
  const result = preprocessByDocType(input, 'general', { documentName: '공개-제안요청서.txt' });

  assert.equal(result.chunks.length, 4);
  assert.ok(result.chunks[0].includes('[섹션] I 제안 내용'));
  assert.ok(result.chunks[0].includes('C 70% 평가항목별 내용이 적정함'));
  assert.ok(result.chunks[0].includes('D 55% 평가항목별 내용에 보완이 필요함'));
  assert.ok(result.chunks[0].includes('PPT파일 형식, A4 규격'));
  assert.ok(result.chunks[1].includes('[섹션] [서식 제1호]'));
  assert.ok(result.chunks[1].includes('서식 본문입니다.'));
  assert.ok(result.chunks[2].includes('[섹션] [서식 제6호-1]'));
  assert.ok(result.chunks[3].includes('[섹션] [서식 제7호]'));
});
