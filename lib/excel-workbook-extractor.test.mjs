...
test('comment discovery visits only materialized cells at the maximum Excel coordinate', () => {
  let cellReads = 0;
  const sheet = new Proxy({
    '!ref': 'A1:XFD1048576',
    A1: { t: 's', v: 'visible' },
    XFD1048576: { t: 'z', c: [{ a: '검토자', t: '가장 먼 메모' }] },
  }, {
    get(target, property, receiver) {
      if (typeof property === 'string' && /^[A-Z]+\d+$/u.test(property)) {
        cellReads += 1;
        if (cellReads > 3) throw new Error('comment discovery scanned empty coordinates');
      }
      return Reflect.get(target, property, receiver);
    },
  });

  const summary = extractSheetCommentSummary(sheet);

  assert.equal(cellReads <= 2, true);
  assert.deepEqual(summary, { count: 1, locations: ['XFD1048576'] });
});
...
test('warns once for legacy notes and threaded comments without exposing their contents', () => {
  const sheet = XLSX.utils.aoa_to_sheet([['visible'], [], []]);
  sheet.A1.c = [{ a: '민감작성자', t: '비공개 검사 내용' }];
  sheet.B2 = {
    t: 'z',
    c: [
      { a: '검토자 1', t: '첫 번째 스레드 조각', T: true },
      { a: '검토자 2', t: '두 번째 스레드 조각', T: true },
    ],
  };
  sheet.C1000 = { t: 'z', c: [{ a: '검토자 3', t: '빈 셀 메모' }] };
  sheet['!ref'] = 'A1:C1000';
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, sheet, '검사');
  const bytes = XLSX.write(workbook, { type: 'array', bookType: 'xlsx' });

  const document = extractWorkbookDocument(bytes, 'comments.xlsx');
  const warning = document.warnings.find((issue) => issue.code === 'EXCEL_CELL_COMMENTS_UNSUPPORTED');

  assert.equal(warning?.code, 'EXCEL_CELL_COMMENTS_UNSUPPORTED');
  assert.equal(warning?.count, 3);
  assert.deepEqual(warning?.locations, ['검사!A1', '검사!B2', '검사!C1000']);
  assert.equal(JSON.stringify(document).includes('비공개 검사 내용'), false);
  assert.equal(JSON.stringify(document).includes('민감작성자'), false);
});

test('caps comment warning locations at twenty while retaining the total commented-cell count', () => {
  const sheet = XLSX.utils.aoa_to_sheet([['visible']]);
  for (let column = 0; column < 21; column += 1) {
    const address = XLSX.utils.encode_cell({ r: 0, c: column });
    sheet[address] = sheet[address] ?? { t: 'z' };
    sheet[address].c = [{ a: '검토자', t: `메모 ${column + 1}` }];
  }
  sheet['!ref'] = 'A1:U1';
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, sheet, '한도');
  const bytes = XLSX.write(workbook, { type: 'array', bookType: 'xlsx' });

  const document = extractWorkbookDocument(bytes, 'comment-cap.xlsx');
  const warning = document.warnings.find((issue) => issue.code === 'EXCEL_CELL_COMMENTS_UNSUPPORTED');

  assert.equal(warning?.count, 21);
  assert.equal(warning?.locations?.length, 20);
  assert.deepEqual(warning?.locations?.slice(0, 2), ['한도!A1', '한도!B1']);
});

test('keeps a comment-only distant cell out of the displayed range while reporting it', () => {
  const sheet = XLSX.utils.aoa_to_sheet([['visible']]);
  sheet.C1000 = { t: 'z', c: [{ a: '검토자', t: '멀리 있는 메모' }] };
  sheet['!ref'] = 'A1:C1000';
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, sheet, '희소');
  const bytes = XLSX.write(workbook, { type: 'array', bookType: 'xlsx' });

  const document = extractWorkbookDocument(bytes, 'sparse-comments.xlsx');

  assert.deepEqual(document.blocks[0].rows, [['visible']]);
  assert.deepEqual(document.blocks[0].excelLayout?.usedRange, {
    startRow: 1,
    endRow: 1,
    startColumn: 1,
  ...
  assert.deepEqual(
    document.warnings.find((issue) => issue.code === 'EXCEL_CELL_COMMENTS_UNSUPPORTED')?.locations,
    ['희소!C1000'],
  );
});

test('explains that a comment-only workbook must be reviewed in the original Excel file', () => {
  const sheet = {
    '!ref': 'A1:A1',
    A1: { t: 'z', c: [{ a: '검토자', t: '메모만 있습니다' }] },
  };
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, sheet, '메모만');
  const bytes = XLSX.write(workbook, { type: 'array', bookType: 'xlsx' });

  assert.throws(
    () => extractWorkbookDocument(bytes, 'comment-only.xlsx'),
    /메모|주석/u,
  );
});
