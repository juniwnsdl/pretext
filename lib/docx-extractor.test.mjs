...
test('hides Mammoth paragraph-style diagnostics that do not affect extracted content', async () => {
  const document = await extractDocxDocument(
    ...
    async () => ({
      value: '<p>계약 본문</p>',
      messages: [
        { type: 'warning', message: "Unrecognised paragraph style: '일반본문' (Style ID: a0)" },
        { type: 'warning', message: 'A meaningful conversion warning' },
      ],
    }),
  );
  ...
  assert.deepEqual(document.warnings, [{
    code: 'DOCX_CONVERSION_WARNING',
    severity: 'warning',
    message: 'A meaningful conversion warning',
  }]);
});
...
test('rejects a colspan beyond the logical column budget before grid allocation', () => {
  assert.throws(
    () => parseMammothHtml(
      '<table><tr><td colspan="10000">Hostile width</td></tr></table>',
      'huge-colspan.docx',
    ),
    { message: 'DOCX table exceeds safe logical grid limits.' },
  );
});

test('rejects a rowspan beyond the logical row budget before grid allocation', () => {
  assert.throws(
    () => parseMammothHtml(
      '<table><tr><td rowspan="10000">Hostile height</td></tr></table>',
      'huge-rowspan.docx',
    ),
    { message: 'DOCX table exceeds safe logical grid limits.' },
  );
...
test('rejects malformed or non-positive span attributes', () => {
  for (const colspan of ['12oops', '1.5', '0', '-2']) {
    assert.throws(
      () => parseMammothHtml(
        `<table><tr><td colspan="${colspan}">Invalid span</td></tr></table>`,
        'invalid-span.docx',
      ),
      { message: 'DOCX table exceeds safe logical grid limits.' },
    );
...
test('rejects moderate spans whose rectangular grid exceeds the total cell budget', () => {
  const rows = '<tr><td colspan="500">Moderate span</td></tr>'.repeat(201);

  assert.throws(
    () => parseMammothHtml(`<table>${rows}</table>`, 'aggregate-grid.docx'),
    { message: 'DOCX table exceeds safe logical grid limits.' },
  );
});
