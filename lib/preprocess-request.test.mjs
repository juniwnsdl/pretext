...
test('accepts and immutably clones bounded Excel column, formula, and output metadata', () => {
  const excelBlock = {
    id: 'sheet-1',
    kind: 'table',
    order: 0,
    ...
    rows: [['값', '합계'], ['2', '5']],
    excelLayout: {
      usedRange: { startRow: 5, endRow: 6, startColumn: 3, endColumn: 4 },
      headerRows: { startRow: 5, endRow: 5, source: 'manual' },
    },
    formulaCells: [{
      row: 6,
      column: 4,
  ...
  const excelOptions = { formulaOutput: 'value-and-formula' };
  const valid = normalizePreprocessRequest({
    document: structuredDocument({
      sourceFormat: 'xlsx',
      extractionMethod: 'local-excel',
  ...
  assert.equal(valid.ok, true);
  assert.deepEqual(valid.value.document.blocks[0].formulaCells, excelBlock.formulaCells);
  assert.notEqual(valid.value.document.blocks[0].formulaCells, excelBlock.formulaCells);
  assert.notEqual(valid.value.document.blocks[0].formulaCells[0], excelBlock.formulaCells[0]);
  assert.deepEqual(valid.value.document.excelOptions, excelOptions);
  assert.notEqual(valid.value.document.excelOptions, excelOptions);
});

test('rejects invalid Excel column, formula, and output metadata', () => {
  const baseBlock = {
    id: 'sheet-1',
    kind: 'table',
    order: 0,
    ...
    rows: [['값'], ['2']],
    excelLayout: {
      usedRange: { startRow: 5, endRow: 6, startColumn: 3, endColumn: 3 },
      headerRows: { startRow: 5, endRow: 5, source: 'manual' },
    },
  };
  const invalidDocuments = [
    structuredDocument({ blocks: [{ ...baseBlock, excelLayout: {
      ...baseBlock.excelLayout,
      usedRange: { ...baseBlock.excelLayout.usedRange, startColumn: 0 },
    } }] }),
    structuredDocument({ blocks: [{ ...baseBlock, excelLayout: {
      ...baseBlock.excelLayout,
      usedRange: { ...baseBlock.excelLayout.usedRange, endColumn: 2 },
    } }] }),
    structuredDocument({ blocks: [{ ...baseBlock, formulaCells: [{
      row: 0, column: 3, formula: '=C6', hasStoredResult: true,
    }] }] }),
    structuredDocument({ blocks: [{ ...baseBlock, formulaCells: [{
      row: 6, column: 3, formula: 'C6', hasStoredResult: true,
    }] }] }),
    structuredDocument({ blocks: [{ ...baseBlock, formulaCells: [{
      row: 6, column: 3, formula: '=C6', hasStoredResult: 'yes',
    }] }] }),
    structuredDocument({ excelOptions: { formulaOutput: 'formula-only' } }),
  ];
...
test('rejects formula coordinates outside real rows, duplicate coordinates, and mismatched used width', () => {
  const baseBlock = {
    id: 'sheet-1',
    kind: 'table',
    order: 0,
  ...
  const formula = { row: 2, column: 2, formula: '=A2+3', hasStoredResult: true };
  const invalidBlocks = [
    { ...baseBlock, formulaCells: [{ ...formula, column: 99 }] },
    { ...baseBlock, formulaCells: [formula, { ...formula, formula: '=A2+4' }] },
    { ...baseBlock, excelLayout: {
      ...baseBlock.excelLayout,
      usedRange: { startRow: 1, endRow: 2, startColumn: 1, endColumn: 3 },
    } },
  ];

  for (const block of invalidBlocks) {
    const result = normalizePreprocessRequest({ document: structuredDocument({ blocks: [block] }) });
    assert.equal(result.ok, false);
    assert.equal(result.error.code, 'INVALID_BLOCK');
  }
});
